...
"""
RAIS Human Core v2
- 기존 rais_human_core_v1을 보존하면서 AI 분석 보조 레이어를 추가한 V2 엔진
- UI(index.html)는 수정하지 않는 것을 전제로 설계
- app.py에서는 import만 아래처럼 교체하면 됨

from rais_human_core_v2 import (
...
# 0. 기존 v1 엔진 import
# =========================================================
import rais_human_core_v1 as v1
...
try:
    ...
    def build_fallback_result(input_data: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        return {
            "engine": "RAIS_HUMAN_CORE_V2_FALLBACK",
...
MODEL_VERSION = "RAIS_HUMAN_CORE_V2_AI_RA_BALANCED_B_LAYER_2026_05_05"
...
# 3. AI Prompt 생성
# =========================================================
def build_ai_prompt(ra_structure: Dict[str, Any]) -> str:
    """
    RA 구조를 AI 분석용 prompt로 변환.
    V2.1에서는 AI가 단순 설명자가 아니라 RAIS v1 판단을 검토하고,
    ...
    prompt = f"""
당신은 RAIS Human Core V2.1의 보조 판단 엔진입니다.
...
현재 목표/고민:
{compact_json(ctx.get("current_goals", []))}
목표1: {ctx.get("current_goal_1", "")}
목표2: {ctx.get("current_goal_2", "")}
목표3: {ctx.get("current_goal_3", "")}
고민: {ctx.get("concern", "")}
...
def parse_ai_result(text: str) -> Dict[str, Any]:
    ...
    V2.1 JSON 구조와 구버전 JSON 구조를 모두 받아들인다.
...
def integrate_ai_results(
    ...
    V2.1에서는 AI가 단순 문장 보조가 아니라 경로 판단과 핵심 문제를
...
def build_ra_narrative(integrated_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    RAIS Human Core V2.1 스타일의 최종 문장 생성.
...
def build_ra_result(raw_input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # print("RAIS V2 base v1 build_ra_result error:")
        traceback.print_exc()
    ...
    # 3. AI prompt 생성
    prompt = build_ai_prompt(ra_structure)
    ...
    result["ra_structure_v2"] = ra_structure
